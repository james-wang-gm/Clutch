...
class CustomPipelineOptions(PipelineOptions):
    ...
    def _add_argparse_args(cls, parser):
        parser.add_value_provider_argument(
            ...
            default = '{}-analytics-data-lake/Clutch/Rewards_Points_Updated_Dead/PointsUpdatedDead_testing.json'.format(env))
        parser.add_value_provider_argument(
            '--output',
            type=str,
...
def streaming_pipeline(project, region="us-central1"):
    ...
    subscription = "projects/{}/subscriptions/Rewards_Points_Updated_Dead".format(project)
    bucket = "gs://{}-analytics-data-lake/Clutch/Rewards_Points_Updated_Dead/".format(env)
    ...
    options = PipelineOptions(
        ...
        template_location = "gs://{}-analytics-temp-files/Clutch/Clutch_PointsUpdatedDead_PStoGCS.py".format(env),
    ...
    class WriteToGCS(beam.DoFn):
        def __init__(self):
            self.outdir = "gs://{}-analytics-data-lake/Clutch/Rewards_Points_Updated_Dead/".format(env)
        ...
        def process(self, element):
            ...
            file_prefix = "Rewards_Points_Updated_Dead_Output_" + timestampStr + '.json' 
            writer = FileSystems.create(self.outdir + file_prefix, 'text/plain')
            writer.write(element.encode())
            writer.close()

    subscription = "projects/{}/subscriptions/Rewards_Points_Updated_Dead".format(project)
    topic = "projects/{}/topics/rewards-points-updated-dead".format(project)
    bucket = "gs://{}-analytics-data-lake/Clutch/Rewards_Points_Updated_Dead/".format(env)
